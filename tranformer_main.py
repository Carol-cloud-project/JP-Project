...
plt.legend()
plt.title('Loss')
plt.savefig(os.getcwd() + '/result/Transformer_Loss')
plt.show()
# MAE（损失值）
plt.figure()
...
plt.legend()
plt.title('Loss')
plt.savefig(os.getcwd() + '/result/Transformer_MSE')
plt.show()
# 真实值与预测值
...
for i in range(5):
    ...
    plt.savefig(os.getcwd() + '/result/Transformer_'+str(i)+'_True-Pred.png')
    plt.close()
# 模型评估
...
print(result)
result.to_csv(os.getcwd() + '/result/Transformer_result.csv')
pd.DataFrame(y_true).to_csv(os.getcwd() + '/result/Transformer_y_true.csv')
pd.DataFrame(y_pred).to_csv(os.getcwd() + '/result/Transformer_y_pred.csv')
