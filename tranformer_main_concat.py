...
x_data_emb = pd.read_table('/home/rliuaj/balance_sheet/result/Transformer_y_pred_emb_only.csv',sep=',',index_col=0)

x_data = x_data.reset_index(drop=True)
x_data_emb = x_data_emb.reset_index(drop=True)
x_data = pd.concat([x_data, x_data_emb], axis=1)
...
learning_rate = 0.01  # Example: setting learning rate to 0.001
...
plt.legend()
plt.title('Loss')
plt.savefig(os.getcwd() + '/result/Transformer_Loss_concat')
plt.show()
# MAE
plt.figure()
...
plt.legend()
plt.title('Loss')
plt.savefig(os.getcwd() + '/result/Transformer_MSE_concat')
plt.show()
# 真实值与预测值
...
for i in range(5):
    ...
    plt.savefig(os.getcwd() + '/result/Transformer_'+str(i)+'_True-Pred_concat.png')
    plt.close()
# 模型评估
...
print(result)
result.to_csv(os.getcwd() + '/result/Transformer_result_concat.csv')
pd.DataFrame(y_true).to_csv(os.getcwd() + '/result/Transformer_y_true_concat.csv')
pd.DataFrame(y_pred).to_csv(os.getcwd() + '/result/Transformer_y_pred_concat.csv')



#test
random_indices = random.sample(range(len(y_true)), 5)
print("random index", random_indices)

print(y_true)
print(y_true[random_indices])
print(y_pred[random_indices])
print(y_true[random_indices]-y_pred[random_indices])